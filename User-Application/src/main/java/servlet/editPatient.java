...
public class editPatient extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Part patient_mdP = request.getPart("patient_md");
        Part patient_prP = request.getPart("patient_pr");
        System.out.println("Estamos dentro del servlet para para editar el patient");
        ...
        if(patient_mdP != null) dt_md = new StreamDataBodyPart("patient_md",patient_mdP.getInputStream());
        if(patient_prP != null) dt_pr = new StreamDataBodyPart("patient_pr",patient_prP.getInputStream());
        ...
        try (FormDataMultiPart formDataMultiPart = new FormDataMultiPart()) {
            ...
            if(patient_mdP != null) multipart.bodyPart(dt_md);
            if(patient_prP != null) multipart.bodyPart(dt_pr);
            final Client client = ClientBuilder.newBuilder().register(MultiPartFeature.class).build();
            final WebTarget target = client.target(props.getProperty("workflowUrl") + "/api/v1/editPatient");
        ...
        RequestDispatcher dispatcher = request.getServletContext().getRequestDispatcher("/editPatient.jsp");
        dispatcher.forward(request, response);        
    }
